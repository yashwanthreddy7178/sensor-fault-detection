...
### Problem Statement
The Air Pressure System (APS) is a critical component of a heavy-duty vehicle that uses compressed air to force a piston to provide pressure to the brake pads, slowing the vehicle down. The benefits of using an APS instead of a hydraulic system are natural air's easy availability and long-term sustainability.

This is a Binary Classification problem, in which the affirmative class indicates that the failure was caused by a certain component of the APS, while the negative class
indicates that the failure was caused by something else.

### Solution Proposed 
In this project, the system in focus is the Air Pressure system (APS) which generates pressurized air that is utilized in various functions in a truck, such as braking and gear changes. The dataset's positive class corresponds to component failures for a specific component of the APS system. The negative class corresponds to trucks with failures for components unrelated to the APS system.
...
## How to run?
Before we run the project, could you make sure that you have MongoDB in your local system, with Compass since we are using MongoDB for data storage? You also need an AWS account to access the service like S3, ECR, and EC2 instances.
...
To run the project  first execute the below command.
MONGO DB URL: 
```
